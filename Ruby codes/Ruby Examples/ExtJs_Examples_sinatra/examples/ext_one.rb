...
# p params[:search]
	...
	search_word = Word.filter[:words=> search]
	
	# p search_word[:meanings]
		
	 if search_word
	 
	 resp = {
		 'success'=>true,
		 'message'=>"word found- Maening is ::#{search_word[:meanings]}"
	 }
	 
	 else
	 resp = {
		 'success'=>false,
		 'message'=>'word not found try with another word'
	 }
